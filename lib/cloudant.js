...
Cloudant.prototype.all = function all(model, filter, options, cb) {
  ...
  function findAllDocuments(query, cb, resultDocs){
    mo.db.find(query, function(err, result) {
      debug('Cloudant.prototype.all.findAllDocuments results: %j %j', err, result);
      if (err) return cb(err, result);

      resultDocs = resultDocs || [];
      var combinedDocs = resultDocs.concat(result.docs);
      if(result.docs.length === 200){ //Maximum limit of cloudant query
        query.bookmark = result.bookmark;
        findAllDocuments(query, cb, combinedDocs);
      } else {
        cb(err, combinedDocs);
      }
   })
  ...
  findAllDocuments(query, function(err, resultDocs) {
    debug('Cloudant.prototype.all results: %j %j', err, resultDocs);
    if (err) return cb(err, resultDocs);
    if (!options || !options.raw) {
      for (var i = 0; i < resultDocs.length; i++) {
        self.fromDB(model, mo, resultDocs[i]);
      }
    }
    if (filter && filter.include) {
      self._models[model].model.include(resultDocs,
                                        filter.include,
                                        options, cb);
    } else {
      cb(null, resultDocs);
